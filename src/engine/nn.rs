...
impl NeuralNetwork {
    pub fn new() -> NeuralNetwork {
        NeuralNetwork {
            layers: vec![],
            cost_function: CostFunctionArgu::new(CostFunctions::SquaredError,Vec::new()),
            shuffle_data: true,
        }
    }
    ...
    pub fn set_cost_function<T>(&mut self, cost_function: CostFunctionArgu)
    where
        ...
        for (i, layer) in self.layers.iter().enumerate() {
            ...
            if i > 0 {
                let mult: Matrix = prev_weight
                    ...
                    .map_row(&|n| layer.activation.get_activation().calc(n));
    ...
    fn error(&self, prediction: &Matrix, target: &Matrix) -> f64 {
        self.cost_function.get_cost_func().calc(prediction, target)
    }
            ...
            for sample in samples.iter() {
                ...
                for (i, layer) in output.iter().enumerate() {
                    ...
                    let forward_derivative: Matrix =
                        layer.map_row(&|n| self.layers[index].activation.get_activation().derivative(n));
    ...
    pub fn get_neural_from_str(input: Vec<u8>) -> Result<NeuralNetwork,Vec<u8>>{
        Ok(postcard::from_bytes(input.as_slice()).unwrap())
    }
